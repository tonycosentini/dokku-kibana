...
@test "($PLUGIN_COMMAND_PREFIX:link) error when the service is already linked to app" {
  ...
  assert_contains "${lines[*]}" "Already linked as KIBANA_URL"
}

@test "($PLUGIN_COMMAND_PREFIX:link) exports KIBANA_URL to app" {
  dokku "$PLUGIN_COMMAND_PREFIX:link" l my_app
  url=$(dokku config:get my_app KIBANA_URL)
  assert_contains "$url" "http://dokku-kibana-l:5601"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when KIBANA_URL already in use" {
  dokku config:set my_app KIBANA_URL=http://host:5601
  dokku "$PLUGIN_COMMAND_PREFIX:link" l my_app
  run dokku config my_app
  assert_contains "${lines[*]}" "DOKKU_KIBANA_"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.kibana.l:dokku-kibana-l"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}
