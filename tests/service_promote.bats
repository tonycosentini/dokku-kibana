...
@test "($PLUGIN_COMMAND_PREFIX:promote) error when the service is already promoted" {
  ...
  assert_contains "${lines[*]}" "already promoted as KIBANA_URL"
}

@test "($PLUGIN_COMMAND_PREFIX:promote) changes KIBANA_URL" {
  dokku config:set my_app "KIBANA_URL=http://host:9200" "DOKKU_KIBANA_BLUE_URL=http://dokku-kibana-l:5601"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my_app
  url=$(dokku config:get my_app KIBANA_URL)
  assert_equal "$url" "http://dokku-kibana-l:5601"
}

@test "($PLUGIN_COMMAND_PREFIX:promote) creates new config url when needed" {
  dokku config:set my_app "KIBANA_URL=http://host:9200" "DOKKU_KKIBANA_BLUE_URL=http://dokku-kibana-l:5601"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my_app
